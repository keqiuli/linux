...
typedef struct regelement{
        perm_string 	name;      //REG的名字
        unsigned int	base;    //reg 驱动起始地址
        unsigned int 	width;     //reg 驱动宽度
        unsigned int 	forstart;  //reg for赋值起始
        unsigned int 	forwidth;  //reg for赋值宽度
	searchstack 	sstack;	   //分支深度
	unsigned int 	lino;	   //变量所在行数	
}regelement;

//被搜索变量 
typedef struct reg_search_ele{
        perm_string  name;
        unsigned int width;
}reg_search_ele;
//
...
class Design {
	...
	// Iterate over the design...
      ...
      void check_confict();
      void check_driver_conflict(const NetProc* pro, reg_search_ele ele, list<regelement>* regmem, regelement memele);
